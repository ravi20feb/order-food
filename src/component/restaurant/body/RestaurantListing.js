...
const BACKGROUND = {
  BebkitBoxShadow: 'inset 0px -110px 65px -38px rgba(0,0,0,0.75)',
...
//   dynamically set isSelector value
    ...
    return(
        <div className="grid-listing-wrapper body-margin border-2 border-orange ">
            <div>
            
                <div className="grid-container">
                    <div className="grid grid-cols-[repeat(4,_1fr)] gap-4 my-8 mx-3 items-start xs:grid-cols-[repeat(1,_1fr)] sm:grid-cols-[repeat(2,_1fr)] md:grid-cols-[repeat(3,1fr)] xs:gap-2   lg:min-w-[auto] lg:max-w-[100vw]">
                        {
                                ...
                                isSelector.map((info)=>{
                                        return( 
                                            <Link  to={'/home/restaurant'+'/'+info?.info?.id}>
                                                <div key={info.info.id} className=" leading-[1.75rem] relative cursor-pointer hover:scale-[.95] xs:hover:scale-1 lg:mx-auto lg:min-w-[auto] lg:max-w-[100vw]  transition-all">
                                                {/* {console.log(carouselCard)} */}
                                                    ...
                                                    <div className="  flex flex-col w-[14rem]  relative   lg:mx-auto xs:w-[80vw] ">
                                                        <div className="caro1-img   mx-0 my-0 relative  rounded-[1.2rem]  overflow-hidden	" >
                                                            <div className="  w-[24rem] left-0 right-0  h-[9rem] absolute  z-[1] xs:h-[13rem] " style={BACKGROUND}></div>
                                                            
                                                                <img className= "bg-cover w-[full]  h-[9rem] xs:w-[full] xs:h-[12rem]  "     src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_850,h_504/`+`${info.info.cloudinaryImageId}`} />
                                                            <div className=" absolute bottom-2 mr-1 truncate font-[800] text-[1.2rem] text-[white] px-3 z-[100]  text-left h-10 w-full overflow-hidden  ">{info?.info?.aggregatedDiscountInfoV3?.header ? info?.info?.aggregatedDiscountInfoV3?.header +" " +info?.info?.aggregatedDiscountInfoV3?.subHeader :''}</div>
                            ...
                            {onScrollRestaurant && <InfiniteShimmer />}
                    </div>
